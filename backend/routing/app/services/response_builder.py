...
from app.schemas import RouteContext
...
def build_response(
    ...
    main_total_length = (
        route["total_length"] + route_context["dist_start"] + route_context["dist_end"]
    )
    main_total_time = route["total_time"] + start_time + end_time
    ...
    if include_alternatives and alternatives:
        ...
        for idx, alt_route in enumerate(alternatives, start=1):
            ...
            alt_total_time = alt_route["total_time"] + start_time + end_time
