...
from app.schemas import EdgeData, HighWayType, RoadCategory, Speed
...
def pick_closer_node(G, u: int, v: int, point: tuple[float, float]) -> int:
    ...
    du = ox.distance.great_circle(G.nodes[u]["y"], G.nodes[u]["x"], point[0], point[1])
    dv = ox.distance.great_circle(G.nodes[v]["y"], G.nodes[v]["x"], point[0], point[1])
